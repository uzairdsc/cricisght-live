...
LIVE_DATA_DIR = Path(DATA_ROOT) / "live-data"
SQUAD_DATA_DIR = Path(DATA_ROOT) / "squads"
live_data_path = Path(LIVE_DATA_DIR) / "WWT20-26_1490682-Comm.csv"
...
if data_source == "Upload Data File":
    ...
    if "aws" in st.secrets:
        ...
        s3_file_key = st.sidebar.text_input("Enter S3 file path:", value="WWT20-26_1490682-Comm.csv")

        if st.sidebar.button("Load from S3", key="load_s3"):
    ...
    local_file_path = st.sidebar.text_input("Enter local file path:", value=str(live_data_path))

    if st.sidebar.button("Load from Local Storage", key="load_cache"):
        try:
...
if st.session_state.df is not None:
    ...
    # List of possible file paths (in order of preference)
    # possible_paths = [
    #     SQUAD_DATA_DIR + "/S2026_WWT20.xlsx",
    #     SQUAD_DATA_DIR + "S2026_PSL.xlsx",
    # ]
